BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = "Im_Herzen_von_Asien_Band_I"
TITLE_NAME = "Im Herzen von Asien Band I"
...
$(BUILD)/pdf/$(OUTPUT_FILENAME).pdf: $(MAKEFILE) $(METADATA) $(CHAPTERS) $(CSS_FILE) $(IMAGES) $(COVER_IMAGE) $(METADATA_PDF) $(PREFACE_EPUB)
	mkdir -p $(BUILD)/pdf
	cp  *.css  $(IMAGES_FOLDER)
	cp  $(IMAGES_FOLDER)/Im_Herzen_von_Asien_I_*.jpg .
